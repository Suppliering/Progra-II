...
public class InputOutput {
	...
	public static DiccionarioMultipleTDA estacionesConLineaTransferencia;
	public static DiccionarioMultipleTDA estacionesIguales;
	public static DiccionarioMultipleTDA lineaCantidadTransferecias;
	...
	public static void generarTDAsYEstructuras() {
		...
		estacionesConLineaTransferencia = new DiccionarioMultiple();
		estacionesIguales = new DiccionarioMultiple();
		lineaCantidadTransferecias = new DiccionarioMultiple();
		estacionesConLineaTransferencia.inicializarDiccionario();
		lineaEstaciones.inicializarDiccionario();
		estacionesIguales.inicializarDiccionario();
		lineaCantidadTransferecias.inicializarDiccionario();
			...
			while((register = buffer.readLine()) != null) {
				...
				if(!"".equals(campos[2])) {
					...
					if(!"".equals(campos[4])) {
						...
						estacionesConLineaTransferencia.agregar((estacion*1000)+linea, lineaTransferencia);
						lineaCantidadTransferecias.agregar(linea, estacion);
					}
				}
			}
			arch.close();		
	...
	public static void listarTransferenciaLineas() {
		...
		ConjuntoTDA estacionesConLineaTransferencias = estacionesConLineaTransferencia.claves();

		System.out.println("Estacion - Linea - Cant. de posibilidades");
		while(!estacionesConLineaTransferencias.conjuntoVacio()) {

			int tam = 0;
			int cl = estacionesConLineaTransferencias.elegir();
			ConjuntoTDA lineasTransferencias = estacionesConLineaTransferencia.recuperar(cl);
			...
			estacionesConLineaTransferencias.sacar(cl);
		}
		...
		while(!aux.colaVacia()) {
			...
			int estacion = aux.primero() / 1000;
			int linea = aux.primero() % 1000;
			
			String estacionTransferencia = obtenerNombreEstacion(estacion);
			String nombreLinea = obtenerNombreLinea(linea);

			System.out.println(estacionTransferencia + " - " + nombreLinea + " - " + aux.prioridad());
			aux.desacolar();
		}
		System.out.println();
	}
	...
	public static void mostrarPorcentajeTransferenciasLinea() {
		
		System.out.println("Porcentaje de estaciones de transferencia");
		ConjuntoTDA claves = lineaEstaciones.claves();
		while(!claves.conjuntoVacio()) {
			...
			int tam2 = 0;
			ConjuntoTDA estaTransf = lineaCantidadTransferecias.recuperar(cl);
			while(!estaTransf.conjuntoVacio()) {
				estaTransf.sacar(estaTransf.elegir());
				tam2++;
			}
			...
			System.out.println("Linea " + linea + ": " +  BigDecimal.valueOf(((float)((tam2)*100)/tam)).setScale(1, BigDecimal.ROUND_HALF_UP).floatValue() +  "%");
